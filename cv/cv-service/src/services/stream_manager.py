...
class StreamManager:
    ...
    def get_active_streams(self) -> List[dict]:
        ...
        streams = []
        for cam in cameras:
            # Simulated check on feed ping status
            streams.append({
                "camera_id": cam.id,
    ...
    def capture_frame(self, rtsp_url: str) -> Optional[np.ndarray]:
        ...
        # Simulated Frame Extraction (returns mock 640x480 RGB image canvas)
    ...
    def preprocess_frame(self, frame: np.ndarray, target_size: tuple = (640, 640)) -> np.ndarray:
        # Resize frame using letterbox/interpolation configurations
        resized = cv2.resize(frame, target_size, interpolation=cv2.INTER_LINEAR)
        # Normalize pixel ranges to [0, 1] standard float layouts
